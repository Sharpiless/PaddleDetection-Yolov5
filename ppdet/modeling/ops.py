...
def yolo_box_v5(
    ...
    #          conf_thresh, 'downsample_ratio', downsample_ratio, 'clip_bbox',
    #          clip_bbox, 'scale_x_y', scale_x_y)
    boxes, scores = ops_yolo_box_v5(
        x, origin_shape, anchors, class_num,
        conf_thresh, downsample_ratio, clip_bbox, scale_x_y
    )
...
def ops_clip_bbox(boxes, im_shape):
    h, w = im_shape[0], im_shape[1]
    x1 = boxes[:, 0].clip(0, w)
    y1 = boxes[:, 1].clip(0, h)
    x2 = boxes[:, 2].clip(0, w)
    y2 = boxes[:, 3].clip(0, h)
    return paddle.stack([x1, y1, x2, y2], dim=1)


def ops_yolo_box_v5(x, origin_shape, anchors,
                    class_num=4, conf_thresh=0.05, downsample_ratio=32,
                    clip_bbox=True, scale_x_y=1.0):
    bs, nG = x.shape[0], x.shape[-1]
    # stride = origin_shape / nG
    nA = len(anchors)
    p = x.view(bs, nA, class_num + 5, nG,
               nG).permute(0, 1, 3, 4, 2).contiguous()
    io = p.clone().sigmoid()  # inference output
    io[..., :4] *= origin_shape
    x, y = io[:, :, :, :, 0], io[:, :, :, :, 1]
    w, h = io[:, :, :, :, 2], io[:, :, :, :, 3]
    io = io.view(-1, class_num + 5)
    idx = io[:, 4] > conf_thresh
    if len(idx):
        obj = io[idx][:, 4].view(-1, 1)
        boxes, scores = io[idx][:, :4], io[idx][:, 5:] * obj
        boxes = xywh2xyxy(boxes)
        if clip_bbox:
            boxes = ops_clip_bbox(boxes, paddle.squeeze(origin_shape))
        return boxes, scores
    else:
        return None, None
